...
const BG     = "linear-gradient(180deg,var(--rose) 0%,var(--pink-light) 30%,var(--pink) 65%,#db2777 100%)";
    ...
    <section style={{position:"relative",width:"100%",minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:"clamp(4rem,8vh,7rem) clamp(1rem,4vw,3rem)",background:BG,overflow:"hidden"}}>
      ...
      <motion.div initial={{opacity:0,y:28}} whileInView={{opacity:1,y:0}} viewport={{once:true}}
        ...
        <div style={{textAlign:"center",marginBottom:"2.5rem"}}>
          ...
          <p style={{fontFamily:SANS,fontSize:"0.88rem",color:"rgba(var(--pink-deep-rgb),.6)",margin:0}}>
        ...
        <AnimatePresence>
          {saved&&(
            <motion.div initial={{opacity:0,y:-10,scale:0.96}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:-10}}
              style={{background:"rgba(var(--pink-deep-rgb),.08)",border:"1px solid rgba(var(--pink-deep-rgb),.3)",borderRadius:12,padding:"0.9rem 1.2rem",marginBottom:"1.2rem",textAlign:"center",fontFamily:SANS,fontSize:"0.88rem",color:ACC}}>
        ...
        {!composing&&(
          <motion.button onClick={()=>setComposing(true)}
            ...
            style={{width:"100%",padding:"1.2rem",marginBottom:"1.5rem",background:"rgba(255,255,255,.85)",border:"1.5px dashed rgba(var(--pink-deep-rgb),.4)",borderRadius:18,cursor:"pointer",fontFamily:SERIF,fontStyle:"italic",fontSize:"1.05rem",color:ACC,display:"flex",alignItems:"center",justifyContent:"center",gap:"0.6rem",backdropFilter:"blur(8px)",boxShadow:"0 4px 20px rgba(var(--pink-deep-rgb),.1)"}}>
        ...
        <AnimatePresence>
          {composing&&(
            <motion.div initial={{opacity:0,height:0}} animate={{opacity:1,height:"auto"}} exit={{opacity:0,height:0}}
              ...
              <div style={{background:"rgba(255,255,255,.88)",border:"1px solid rgba(var(--pink-deep-rgb),.2)",borderRadius:22,padding:"1.8rem",boxShadow:"0 8px 40px rgba(var(--pink-deep-rgb),.12)",backdropFilter:"blur(12px)"}}>

                <p style={{fontFamily:SANS,fontSize:"0.68rem",color:"rgba(var(--pink-deep-rgb),.5)",letterSpacing:"0.18em",textTransform:"uppercase",margin:"0 0 0.4rem"}}>From</p>
                <input value={from} onChange={e=>setFrom(e.target.value)} placeholder="your name"
                  style={{width:"100%",padding:"0.75rem 1rem",border:"1px solid rgba(var(--pink-deep-rgb),.2)",borderRadius:10,fontFamily:SANS,fontSize:"0.92rem",color:"#4a1628",outline:"none",background:"rgba(var(--pink-light-rgb),.3)",boxSizing:"border-box",marginBottom:"1rem",caretColor:ACC}}/>

                <p style={{fontFamily:SANS,fontSize:"0.68rem",color:"rgba(var(--pink-deep-rgb),.5)",letterSpacing:"0.18em",textTransform:"uppercase",margin:"0 0 0.4rem"}}>Unlock date</p>
                <input type="date" value={unlockDate} onChange={e=>setUnlockDate(e.target.value)}
                  min={new Date().toISOString().slice(0,10)}
                  style={{width:"100%",padding:"0.75rem 1rem",border:"1px solid rgba(var(--pink-deep-rgb),.2)",borderRadius:10,fontFamily:SANS,fontSize:"0.92rem",color:"#4a1628",outline:"none",background:"rgba(var(--pink-light-rgb),.3)",boxSizing:"border-box",marginBottom:"1rem"}}/>

                <p style={{fontFamily:SANS,fontSize:"0.68rem",color:"rgba(var(--pink-deep-rgb),.5)",letterSpacing:"0.18em",textTransform:"uppercase",margin:"0 0 0.4rem"}}>Your letter</p>
                <textarea value={letter} onChange={e=>setLetter(e.target.value)}
                  placeholder={"Dear future us,\n\nI wanted you to know…"}
                  rows={8}
                  style={{width:"100%",padding:"1rem",border:"1px solid rgba(var(--pink-deep-rgb),.2)",borderRadius:12,resize:"vertical",fontFamily:SERIF,fontSize:"clamp(0.95rem,2vw,1.08rem)",color:"#4a1628",outline:"none",background:"rgba(255,248,252,.6)",boxSizing:"border-box",lineHeight:1.95,caretColor:ACC,marginBottom:"1rem",backgroundImage:"repeating-linear-gradient(transparent,transparent 31px,rgba(var(--pink-deep-rgb),.06) 31px,rgba(var(--pink-deep-rgb),.06) 32px)"}}/>

                {/* Photo upload */}
                <p style={{fontFamily:SANS,fontSize:"0.68rem",color:"rgba(var(--pink-deep-rgb),.5)",letterSpacing:"0.18em",textTransform:"uppercase",margin:"0 0 0.4rem"}}>Photo (optional)</p>
                ...
                {photoUrl ? (
                  ...
                  <motion.button onClick={()=>fileRef.current?.click()} disabled={photoUploading}
                    ...
                    style={{width:"100%",padding:"0.75rem",marginBottom:"1rem",border:"1px dashed rgba(var(--pink-deep-rgb),.3)",borderRadius:10,background:"rgba(var(--pink-light-rgb),.2)",color:"rgba(var(--pink-deep-rgb),.6)",fontFamily:SANS,fontSize:"0.85rem",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:"0.5rem"}}>
                ...
                <div style={{display:"flex",gap:"0.8rem"}}>
                  ...
                  <motion.button onClick={()=>{setComposing(false);setPhotoUrl("");}} whileHover={{scale:1.02}} whileTap={{scale:0.97}}
                    style={{padding:"0.95rem 1.2rem",borderRadius:12,border:"1px solid rgba(var(--pink-deep-rgb),.25)",background:"transparent",color:"rgba(var(--pink-deep-rgb),.6)",fontFamily:SANS,fontSize:"0.88rem",cursor:"pointer"}}>
                    cancel
                  </motion.button>
                </div>
              </div>
        ...
        {pending.length>0&&(
          <div style={{marginBottom:"1.5rem"}}>
            <p style={{fontFamily:SANS,fontSize:"0.68rem",color:"rgba(var(--pink-deep-rgb),.5)",letterSpacing:"0.16em",textTransform:"uppercase",margin:"0 0 0.8rem"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:"0.6rem"}}>
              {pending.map((p,i)=>{
                ...
                return (
                  <div key={i} style={{background:"rgba(255,255,255,.7)",border:"1px solid rgba(var(--pink-deep-rgb),.18)",borderRadius:14,overflow:"hidden",backdropFilter:"blur(8px)"}}>
                    {p.imageUrl && (
                    ...
                    <div style={{padding:"0.9rem 1.1rem",display:"flex",alignItems:"center",gap:"0.8rem"}}>
                      ...
                      <div style={{flex:1}}>
                        ...
                        <p style={{fontFamily:SANS,fontSize:"0.7rem",color:"rgba(var(--pink-deep-rgb),.5)",margin:"0.1rem 0 0"}}>
                          {days>0?`${days} day${days!==1?"s":""} to go`:"unlocking soon…"}
                        </p>
                      </div>
                      <div style={{width:48,height:4,borderRadius:2,background:"rgba(var(--pink-deep-rgb),.12)",overflow:"hidden",flexShrink:0}}>
                      ...
                      <motion.button onClick={()=>deletePending(i)}
                        ...
                        title="delete"
                        style={{background:"none",border:"none",cursor:"pointer",color:"rgba(var(--pink-deep-rgb),.3)",fontSize:"1rem",padding:"2px 4px",flexShrink:0}}>
                        ×
                      </motion.button>
                    </div>
                  </div>
              ...
              {unlocked.map(c=>{
                ...
                return (
                  <motion.div key={c.id} layout
                    style={{background:"rgba(255,255,255,.78)",border:`1px solid ${isOpen?"rgba(var(--pink-deep-rgb),.4)":"rgba(var(--pink-deep-rgb),.18)"}`,borderRadius:18,overflow:"hidden",boxShadow:isOpen?"0 8px 32px rgba(var(--pink-deep-rgb),.15)":"0 2px 12px rgba(var(--pink-deep-rgb),.06)",backdropFilter:"blur(8px)",transition:"border 0.3s,box-shadow 0.3s"}}>
                    <div onClick={()=>setOpened(isOpen?null:c.id)}
                      ...
                      <div style={{flex:1}}>
                        ...
                        <p style={{fontFamily:SANS,fontSize:"0.7rem",color:"rgba(var(--pink-deep-rgb),.45)",margin:"0.1rem 0 0"}}>
                          Unlocked {fmt(c.unlockDate)}
                        </p>
                      </div>
                      <span style={{color:"rgba(var(--pink-deep-rgb),.35)",fontSize:"0.8rem"}}>{isOpen?"▲":"▼"}</span>
                    </div>
                    <AnimatePresence>
                      {isOpen&&(
                        <motion.div initial={{height:0,opacity:0}} animate={{height:"auto",opacity:1}} exit={{height:0,opacity:0}}
                          ...
                          <div style={{padding:"0 1.5rem 1.6rem",backgroundImage:"repeating-linear-gradient(transparent,transparent 31px,rgba(var(--pink-deep-rgb),.05) 31px,rgba(var(--pink-deep-rgb),.05) 32px)"}}>
                            ...
                            <p style={{fontFamily:SANS,fontSize:"0.8rem",color:"rgba(var(--pink-deep-rgb),.4)",textAlign:"right",marginTop:"1rem",marginBottom:0,fontStyle:"italic"}}>
                              — with love 🩷
                            </p>
                          </div>
                        </motion.div>
        ...
        {unlocked.length===0&&pending.length===0&&!composing&&(
          <div style={{textAlign:"center",padding:"3rem 1rem"}}>
            ...
            <p style={{fontFamily:SANS,fontSize:"0.9rem",color:"rgba(var(--pink-deep-rgb),.45)",margin:0}}>No letters yet — write the first one</p>
          </div>
        )}
      </motion.div>
    </section>
