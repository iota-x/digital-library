...
export default function PushPrompt() {
  ...
  // in that case we show an "add to Home Screen" nudge instead of a dead ask.
  ...
  useEffect(() => {
    const onSuggest = (e: Event) => {
      ...
      // Respect prior "asked"/"later" choices first (shared across both paths).
      try {
      ...
      // iOS Safari (not installed): web push is unavailable in a browser tab —
      // the Notification API often isn't even present — so nudge them to install
      // instead of bailing silently (otherwise iPhone users never discover it).
  ...
  const enable = async () => {
    // Safety net: never fire a doomed permission request inside an iOS browser tab.
    if (isIOS() && !isStandalone()) { setIosInstall(true); return; }
    setBusy(true);
    try {
  ...
  return (
    <AnimatePresence>
      {visible && (
        ...
        >
          <span style={{ fontSize: "1.8rem" }}>{iosInstall ? "📲" : "🔔"}</span>
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{ fontFamily: SERIF, fontStyle: "italic", fontSize: "0.95rem", color: "var(--pink-deep)", margin: "0 0 0.15rem", fontWeight: 600 }}>
              {iosInstall ? "get notified on iPhone" : "get a heads up?"}
            </p>
            <p style={{ fontFamily: SANS, fontSize: "0.78rem", color: "var(--text)", margin: 0, lineHeight: 1.4 }}>
              {iosInstall
                ? "Add Us to your Home Screen first — tap Share ⬆️ then “Add to Home Screen”, and open it from there to turn on notifications."
                : reason}
            </p>
          </div>
            ...
            {iosInstall ? (
              <button onClick={never}
                style={{
                  ...
                  border: "none", borderRadius: 50, padding: "0.45rem 0.9rem", cursor: "pointer",
                  boxShadow: "0 4px 12px rgba(var(--pink-deep-rgb),.3)",
                }}>
                got it
              </button>
            ) : (
