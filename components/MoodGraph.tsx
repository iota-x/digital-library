...
const MOOD_COLOR: Record<string,string> = {
  "🥰":"#f472b6","😊":"#fb923c","🥺":"#a78bfa","😂":"#facc15",
  "🌙":"#818cf8","💗":"#ec4899","✨":"#e879f9","🎮":"#34d399",
  "🌷":"#86efac","😴":"#94a3b8","🤭":"#f9a8d4","💫":"#c084fc",
};
...
export default function MoodGraph() {
  ...
  useEffect(()=>{
    ...
    filtered.forEach(d=>{
      const x=toX(d.date.getTime()), y=toY(d.mood);
      const col=MOOD_COLOR[d.mood]||"#f9a8d4";
      /* Glow */
      const radGrad=ctx.createRadialGradient(x,y,0,x,y,14);
      radGrad.addColorStop(0,col.replace(")",",0.25)").replace("rgb","rgba")||"rgba(244,114,182,0.2)");
      ...
      <motion.div initial={{opacity:0,y:28}} whileInView={{opacity:1,y:0}} viewport={{once:true}}
        ...
        <div style={{textAlign:"center",marginBottom:"2.5rem"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:"1rem",marginBottom:"0.8rem"}}>
            <div style={{width:50,height:1,background:"linear-gradient(90deg,transparent,rgba(244,114,182,0.5))"}}/>
            <motion.span style={{fontSize:"1.5rem"}}
              animate={{rotate:[0,360]}} transition={{repeat:Infinity,duration:20,ease:"linear"}}>✦</motion.span>
            <div style={{width:50,height:1,background:"linear-gradient(90deg,rgba(244,114,182,0.5),transparent)"}}/>
          </div>
          <h2 style={{fontFamily:SERIF,fontStyle:"italic",fontSize:"clamp(1.5rem,4vw,2.2rem)",color:"#fce7f3",margin:"0 0 0.4rem",fontWeight:400}}>
            mood constellation
          </h2>
          <p style={{fontFamily:MONO,fontSize:"0.75rem",color:"rgba(244,114,182,0.4)",margin:0,letterSpacing:"0.18em",textTransform:"uppercase"}}>
          ...
          {([["30","30 days"],["90","90 days"],["all","all time"]] as [typeof viewRange,string][]).map(([v,l])=>(
            <motion.button key={v} onClick={()=>setViewRange(v)}
              ...
              style={{
                ...
                letterSpacing:"0.1em",
                border:`1px solid ${viewRange===v?"rgba(236,72,153,0.6)":"rgba(244,114,182,0.2)"}`,
                background:viewRange===v?"rgba(236,72,153,0.15)":"transparent",
                color:viewRange===v?"#f9a8d4":"rgba(244,114,182,0.4)",
                transition:"all 0.2s",
              }}>{l}</motion.button>
          ))}
        </div>
        ...
        <div style={{
          ...
          boxShadow:"0 0 60px rgba(236,72,153,0.05), inset 0 0 40px rgba(0,0,0,0.3)",
        }}>
          {loaded&&filtered.length>0?(
                ...
                {hovered&&(
                  <motion.div initial={{opacity:0,scale:0.88}} animate={{opacity:1,scale:1}} exit={{opacity:0,scale:0.88}}
                    ...
                    style={{
                      ...
                      border:`1px solid ${MOOD_COLOR[hovered.mood]||"rgba(244,114,182,0.3)"}`,
                    ...
                    <div style={{fontFamily:SERIF,fontStyle:"italic",fontSize:"0.85rem",color:"#fce7f3"}}>
                      {MOOD_LABEL[hovered.mood]||""}
                    </div>
                    <div style={{fontFamily:MONO,fontSize:"0.68rem",color:"rgba(244,114,182,0.5)",marginTop:"0.2rem",letterSpacing:"0.1em"}}>
                      {hovered.date.toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})}
                    </div>
                    <div style={{fontFamily:MONO,fontSize:"0.65rem",color:"rgba(244,114,182,0.35)",marginTop:"0.1rem"}}>
                      day {hovered.dayNum}
                    </div>
                  </motion.div>
                )}
            ...
            <div style={{height:280,display:"flex",alignItems:"center",justifyContent:"center",color:"rgba(244,114,182,0.3)",fontFamily:MONO,fontSize:"0.8rem",letterSpacing:"0.12em"}}>
        ...
        {topMoods.length>0&&(
          <div style={{marginTop:"1.8rem"}}>
            <p style={{fontFamily:MONO,fontSize:"0.7rem",color:"rgba(244,114,182,0.4)",letterSpacing:"0.16em",textTransform:"uppercase",margin:"0 0 1rem",textAlign:"center"}}>
              most felt
            </p>
              ...
              {topMoods.map(([mood,count])=>(
                <motion.div key={mood} whileHover={{scale:1.08,y:-3}}
                  style={{
                    ...
                    background:`rgba(${hexToRgb(MOOD_COLOR[mood]||"#f9a8d4")},0.1)`,
                    border:`1px solid rgba(${hexToRgb(MOOD_COLOR[mood]||"#f9a8d4")},0.3)`,
                    borderRadius:30,padding:"0.4rem 0.9rem",
                    boxShadow:`0 0 12px rgba(${hexToRgb(MOOD_COLOR[mood]||"#f9a8d4")},0.15)`,
                  }}>
                  <span style={{fontSize:"1.1rem"}}>{mood}</span>
                  <span style={{fontFamily:SERIF,fontStyle:"italic",fontSize:"0.82rem",color:MOOD_COLOR[mood]||"#f9a8d4"}}>{MOOD_LABEL[mood]}</span>
                  <span style={{fontFamily:MONO,fontSize:"0.7rem",color:"rgba(244,114,182,0.45)"}}>×{count}</span>
                </motion.div>
              ))}
            </div>
          </div>
