      ...
      {media.length > 1 && (
        <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
          ...
          <div style={{ display: "flex", gap: "0.45rem", alignItems: "center" }}>
            {media.map((_, i) => (
              <div key={i} onClick={() => setTopIdx(i)} style={{ width: i === topIdx ? 9 : 5, height: i === topIdx ? 9 : 5, borderRadius: "50%", cursor: "pointer", background: i === topIdx ? "#ec4899" : "rgba(249,168,212,.3)", transition: "all 0.2s" }} />
            ))}
          </div>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "0.45rem" }}>
                    {MOODS.map(m => (
                      <motion.button key={m} onClick={() => setDraft(d => ({ ...d, mood: d.mood === m ? "" : m }))}
                        ...
                        style={{ fontSize: "1.5rem", background: draft.mood === m ? "rgba(236,72,153,.18)" : "rgba(255,255,255,.04)", border: `1.5px solid ${draft.mood === m ? "rgba(236,72,153,.55)" : "rgba(255,255,255,.06)"}`, borderRadius: 10, padding: "0.35rem", cursor: "pointer", transition: "all 0.15s" }}>{m}</motion.button>
                    ))}
                  </div>
                </div>
