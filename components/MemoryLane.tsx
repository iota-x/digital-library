    ...
    <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={onClose}
      ...
      {srcs.length>1&&<>
        ...
        <div style={{position:"absolute",bottom:"1.2rem",left:"50%",transform:"translateX(-50%)",display:"flex",gap:6}}>
          {srcs.map((_,j)=><div key={j} onClick={e=>{e.stopPropagation();setI(j);}} style={{width:j===i?10:6,height:j===i?10:6,borderRadius:"50%",background:j===i?"#ec4899":"rgba(249,168,212,.4)",cursor:"pointer",transition:"all .2s"}}/>)}
        </div>
      </>}
    ...
    <>
      ...
      <motion.div
        ...
        style={{
          ...
          background:'linear-gradient(165deg,#fff5f8 0%,#fce7f3 60%,#fff0f6 100%)',
          borderRadius:'26px 26px 0 0',
          boxShadow:'0 -8px 60px rgba(190,24,93,.18),0 -2px 0 rgba(244,114,182,.25),0 -40px 100px rgba(0,0,0,.28)',
        }}
      >
        {/* drag handle */}
        <div style={{display:'flex',justifyContent:'center',paddingTop:'0.75rem',paddingBottom:'0.2rem',cursor:'pointer'}} onClick={onClose}>
          <motion.div whileHover={{scaleX:1.3,background:'rgba(190,24,93,.4)'}} transition={{duration:0.2}}
            style={{width:44,height:5,borderRadius:99,background:'rgba(190,24,93,.22)'}}/>
        </div>

        {/* header */}
        <div style={{padding:'0.55rem 1.5rem 0.85rem',borderBottom:'1px solid rgba(244,114,182,.12)'}}>
          <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:'0.8rem'}}>
            <div style={{flex:1,minWidth:0}}>
              <div style={{display:'flex',alignItems:'center',gap:'0.42rem',flexWrap:'wrap',marginBottom:'0.2rem'}}>
                ...
                {entry.special&&(
                  <span style={{fontFamily:SANS,fontSize:'0.58rem',background:'linear-gradient(135deg,#fda4af,#ec4899)',color:'#fff',borderRadius:20,padding:'0.12rem 0.55rem',fontWeight:700}}>
                    ✨ {entry.specialLabel||'special moment'}
                  </span>
                )}
              </div>
              <p style={{fontFamily:SERIF,fontStyle:'italic',fontSize:'0.82rem',color:'rgba(157,23,77,.5)',margin:0}}>
                {fmtDate(entry.date)}
              </p>
            </div>
            <motion.button onClick={onClose} whileHover={{scale:1.1,rotate:90}} whileTap={{scale:0.9}}
              transition={{type:'spring',stiffness:300,damping:20}}
              style={{flexShrink:0,width:32,height:32,borderRadius:'50%',border:'1px solid rgba(190,24,93,.18)',background:'rgba(252,231,243,.8)',color:PINK,cursor:'pointer',fontSize:'0.82rem',display:'flex',alignItems:'center',justifyContent:'center'}}>
              ✕
            </motion.button>
          </div>
        </div>
        ...
        {(note || pinned) && (
          <div style={{padding:'1.5rem 1.6rem 2.4rem'}}>
            <div style={{display:'flex',alignItems:'center',gap:'0.8rem',marginBottom:'1.3rem'}}>
              <div style={{flex:1,height:1,background:'linear-gradient(90deg,rgba(190,24,93,.18),transparent)'}}/>
              <motion.span animate={{rotate:[-5,5,-5]}} transition={{repeat:Infinity,duration:2.8}} style={{fontSize:'1.1rem'}}>💌</motion.span>
              <div style={{flex:1,height:1,background:'linear-gradient(270deg,rgba(190,24,93,.18),transparent)'}}/>
            </div>
            {pinned && pinned !== note && (
              <div style={{background:'rgba(249,168,212,.1)',border:'1px solid rgba(244,114,182,.2)',borderRadius:14,padding:'0.9rem 1.1rem',marginBottom:'1.2rem'}}>
                <p style={{fontFamily:SANS,fontSize:'0.56rem',color:'rgba(190,24,93,.45)',letterSpacing:'0.14em',textTransform:'uppercase',margin:'0 0 0.3rem'}}>📌 pinned</p>
            ...
            <div style={{display:'flex',alignItems:'center',gap:'0.8rem',marginTop:'1.6rem'}}>
              <div style={{flex:1,height:1,background:'linear-gradient(90deg,rgba(190,24,93,.12),transparent)'}}/>
              <span style={{fontFamily:SCRIPT,fontSize:'0.95rem',color:'rgba(190,24,93,.45)'}}>— with love 🩷</span>
            </div>
          </div>
        )}
      </motion.div>
...
function MemCard({ entry,cardIdx,glow,onOpen,setRef,isMobile }:{ entry:Entry; cardIdx:number; glow:boolean; onOpen:()=>void; setRef:(el:HTMLDivElement|null)=>void; isMobile:boolean }) {
  ...
  const baseShadow = glow
    ...
    : "0 10px 40px rgba(0,0,0,.18),0 2px 8px rgba(190,24,93,.1)";
  const hoverShadow = glow
    ? "0 0 0 4px #fde68a,0 0 32px rgba(245,158,11,.55),0 0 60px rgba(245,158,11,.2),0 28px 70px rgba(0,0,0,.32)"
    : "0 0 0 1.5px rgba(244,114,182,.55),0 0 22px rgba(249,168,212,.6),0 0 50px rgba(236,72,153,.3),0 0 90px rgba(190,24,93,.14),0 28px 70px rgba(0,0,0,.28)";
    ...
    <div ref={el=>{(wrapRef as any).current=el; setRef(el);}}>
      <motion.div
        ...
        style={{
          ...
          cursor:"pointer",
          border:`1.5px solid ${glow?"#f59e0b":hovered?"rgba(244,114,182,.45)":"rgba(190,24,93,.08)"}`,
          position:"relative",
      ...
      >
          ...
          <>
            ...
            <div style={{padding:"7px 10px 12px",background:"#fff",position:"relative"}}>
              <div style={{display:"flex",alignItems:"center",gap:"0.35rem",flexWrap:"wrap",marginBottom:"2px"}}>
                ...
                {entry.special&&<span style={{fontFamily:SANS,fontSize:"0.52rem",background:"linear-gradient(135deg,#fda4af,#ec4899)",color:"#fff",borderRadius:20,padding:"0.1rem 0.42rem",fontWeight:700}}>✨ {entry.specialLabel||"special"}</span>}
              </div>
              ...
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                <p style={{fontFamily:SANS,fontSize:"0.56rem",color:"rgba(190,24,93,.32)",margin:0,letterSpacing:"0.05em"}}>{fmtDate(entry.date)}</p>
                <motion.span animate={hovered?{x:3,opacity:1}:{x:0,opacity:.5}} transition={{duration:0.2}}
                  style={{fontFamily:SANS,fontSize:"0.56rem",color:"rgba(190,24,93,.5)",letterSpacing:"0.04em",display:"flex",alignItems:"center",gap:2}}>
                  open →
                </motion.span>
              </div>
            </div>
        ...
        {!hasPhoto && (
          <div style={{
            ...
            backgroundImage:"repeating-linear-gradient(transparent,transparent 25px,rgba(190,24,93,.065) 25px,rgba(190,24,93,.065) 26px)",
            borderLeft:"3px solid rgba(249,168,212,.65)",
            position:"relative",
            minHeight:120,
          }}>
            <div style={{position:"absolute",top:9,right:9,background:"rgba(252,231,243,.85)",backdropFilter:"blur(4px)",borderRadius:8,padding:"2px 7px",fontFamily:SCRIPT,fontSize:"0.76rem",color:PINK,border:"1px solid rgba(249,168,212,.4)",lineHeight:1.3}}>
              {entry.mood||"💗"} Day {dn}
            </div>
            {entry.special&&<span style={{display:"inline-block",fontFamily:SANS,fontSize:"0.52rem",background:"linear-gradient(135deg,#fda4af,#ec4899)",color:"#fff",borderRadius:20,padding:"0.1rem 0.45rem",fontWeight:700,marginBottom:"0.45rem"}}>✨ {entry.specialLabel||"special"}</span>}
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              <p style={{fontFamily:SANS,fontSize:"0.56rem",color:"rgba(190,24,93,.3)",margin:0,letterSpacing:"0.05em"}}>{fmtDate(entry.date)}</p>
              {note.length>100&&(
                <motion.span animate={hovered?{x:3,opacity:1}:{x:0,opacity:.45}} transition={{duration:0.2}}
                  style={{fontFamily:SANS,fontSize:"0.56rem",color:"rgba(190,24,93,.5)",letterSpacing:"0.04em"}}>
                  open →
                </motion.span>
              )}
            </div>
  ...
  return (
    <motion.button onClick={onClick} whileHover={{scale:1.06}} whileTap={{scale:0.93}}
      style={{flexShrink:0,padding:"0.34rem 0.88rem",borderRadius:50,cursor:"pointer",
        background:active?"linear-gradient(135deg,#f472b6,#be185d)":"rgba(255,255,255,.75)",
  ...
  return (
    <motion.div initial={{opacity:0}} whileInView={{opacity:1}} viewport={{once:true,margin:"-20px"}}
      ...
      <motion.span animate={{x:[0,8,0],rotate:[0,16,0]}} transition={{repeat:Infinity,duration:3.5,ease:"easeInOut"}}
        style={{fontSize:"1.3rem",flexShrink:0,filter:"drop-shadow(0 1px 6px rgba(190,24,93,.4))"}}>✈️</motion.span>
      <div style={{flex:1,height:2,backgroundImage:"repeating-linear-gradient(90deg,rgba(190,24,93,.28) 0,rgba(190,24,93,.28) 7px,transparent 7px,transparent 16px)",borderRadius:2}}/>
      <span style={{fontFamily:SCRIPT,fontSize:"1.25rem",color:PINK,opacity:.72,whiteSpace:"nowrap",letterSpacing:"0.02em"}}>{label}</span>
      <div style={{flex:1,height:2,backgroundImage:"repeating-linear-gradient(270deg,rgba(190,24,93,.28) 0,rgba(190,24,93,.28) 7px,transparent 7px,transparent 16px)",borderRadius:2}}/>
    </motion.div>
  );
}
...
export default function MemoryLane() {
  ...
  if(loading) return <SectionSkeleton bg="linear-gradient(160deg,#fff5f9,#fce7f3)" accent="rgba(190,24,93,.12)" lines={5}/>;
      ...
      <div style={{position:"relative",zIndex:1,maxWidth:1080,margin:"0 auto"}}>
        ...
        <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{duration:0.6}}
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:"1.3rem",marginBottom:"0.8rem"}}>
            <div style={{flex:1,maxWidth:90,height:1,background:"linear-gradient(90deg,transparent,rgba(190,24,93,.3))"}}/>
            <motion.span animate={{x:[0,10,0],y:[0,-7,0],rotate:[0,20,0]}} transition={{repeat:Infinity,duration:4,ease:"easeInOut"}}
              style={{fontSize:"2.2rem",filter:"drop-shadow(0 3px 12px rgba(190,24,93,.5))"}}>✈️</motion.span>
            <div style={{flex:1,maxWidth:90,height:1,background:"linear-gradient(90deg,rgba(190,24,93,.3),transparent)"}}/>
          </div>
          <h1 style={{fontFamily:SERIF,fontStyle:"italic",fontSize:"clamp(2rem,5vw,3.2rem)",color:PINK,margin:"0 0 0.2rem",fontWeight:400,textShadow:"0 2px 24px rgba(190,24,93,.18)"}}>
            our story so far
          </h1>
          {entries.length>0&&<p style={{fontFamily:SANS,fontSize:"0.82rem",color:"rgba(190,24,93,.42)",margin:"0 0 1.4rem"}}>{entries.length} {entries.length===1?"memory":"memories"} · {totalPhotos} photo{totalPhotos!==1?"s":""}</p>}
          {entries.length>0&&(
            <motion.button onClick={surprise} disabled={spinning}
              whileHover={{scale:1.06,y:-3,boxShadow:"0 16px 44px rgba(190,24,93,.32)"}} whileTap={{scale:0.95}}
              style={{padding:"0.82rem 2.2rem",borderRadius:50,border:"1.5px solid rgba(190,24,93,.3)",background:"rgba(255,255,255,.82)",color:PINK,fontFamily:SERIF,fontStyle:"italic",fontSize:"1rem",cursor:"pointer",backdropFilter:"blur(14px)",boxShadow:"0 4px 22px rgba(190,24,93,.12)",display:"inline-flex",alignItems:"center",gap:"0.5rem"}}>
        ...
        {data.length>0&&(
          <motion.div initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{delay:.12}}
            ...
            <div style={{display:"flex",gap:"0.4rem",flexWrap:"wrap",alignItems:"center",marginBottom:"0.5rem"}}>
              ...
              <div style={{width:1,height:14,background:"rgba(190,24,93,.14)",margin:"0 .15rem"}}/>
              <span style={{fontFamily:SANS,fontSize:"0.6rem",color:"rgba(190,24,93,.38)",letterSpacing:"0.12em",textTransform:"uppercase"}}>group</span>
        ...
        {entries.length>0&&(
          <motion.div initial={{opacity:0}} whileInView={{opacity:1}} viewport={{once:true}}
            style={{textAlign:"center",marginTop:"4rem",paddingTop:"2rem",borderTop:"1px solid rgba(190,24,93,.1)"}}>
